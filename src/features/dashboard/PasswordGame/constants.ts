...
export const initialRules: Rule[] = [
  ...
  { id: 19, text: "Your password must include the word '2FA' as a reminder to enable two-factor authentication.", met: false, revealed: false },
  { id: 20, text: 'Your password must contain at least one even number', met: false, revealed: false },
  { id: 21, text: 'Your password must contain at least one prime number.', met: false, revealed: false },
  ...
  { id: 27, text: 'Your password must contain a number greater than 500.', met: false, revealed: false },
  { id: 28, text: 'Your password must contain no consecutive lowercase letters.', met: false, revealed: false },
  { id: 29, text: "Your password must include the name of a common password-cracking algorithm (Ask teacher if unsure).", met: false, revealed: false },
...
export const checkRules = (password: string, rules: Rule[]): Rule[] => {
  ...
  if (updatedRules[25].revealed) {
    ...
    updatedRules[26].met = numbers ? numbers.some(num => parseInt(num, 10) > 500) : false;
  ...
  if (updatedRules[27].revealed) {
    const algorithms = ['MD5', 'SHA1', 'SHA256', 'bcrypt', 'PBKDF2', 'Argon2', 'NTLM', 'DES', 'Blowfish', 'LM'];
    updatedRules[28].met = algorithms.some(algorithm => password.toLowerCase().includes(algorithm.toLowerCase()));
